...
my $blast = '/usr2/people/gtl/bin/blast64/bin/blastall';
my $myDB = './formatdb/aaseqs';
...
# check homologs
    ...
    my $fastacmd = '/usr2/people/mprice/Genomics/browser/bin/fastacmd';
    system($fastacmd,'-d',$myDB,'-s',$id,'-o',$seqFile)==0 || die "Error running fastacmd: $!";
...
my $database = "./sqlite/feba.db";
my $dbh = DBI->connect("dbi:SQLite:dbname=$database","","",{ RaiseError => 1 }) || die $DBI::errostr;
my ($stmt, $sth, $rv);
